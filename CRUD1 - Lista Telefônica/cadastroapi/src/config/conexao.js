...
const agendamento = {nome:'Dayvison Santos', telefone: '996420722'}
/*inserir */
...
/*selecionar, listar */
con.query('select * from listatelefonica', (err,rows) => {
    if(err) throw err
    console.log('Agendamentos', rows, '\n\n')
})

/*atualizar */
con.query('UPDATE listatelefonica SET nome = ?, telefone = ? where ID = ? ', ['Tiago','12345678','1'], (err,rows) => {
    if(err) throw err
    console.log("Registro Atualizado com sucesso");
})

/*deletar*/
con.query('DELETE FROM listatelefonica where ID = ? ', ['2'], (err,resultado) => {
    if(err) throw err
